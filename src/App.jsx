...
function AuthScreen({ onAuth, toast }) {
  ...
  async function submit() {
    ...
    if (mode === "register") {
      ...
      if (form.ref) {
        const refOwner = Object.values(users).find((u) => u.refCode === form.ref);
        if (refOwner) {
          users[refOwner.phone].balance += REFERRAL_BONUS;
          users[refOwner.phone].referrals += 1;
          users[refOwner.phone].activity = [
            { text: `Referral bonus +KSH ${REFERRAL_BONUS} from new signup`, at: Date.now() },
            ...(users[refOwner.phone].activity || []).slice(0, 19),
          ];
        }
      }
...
// ── Activation Modal ──────────────────────────────────────────────────────────
function ActivationModal({ user, onActivated, onClose }) {
  ...
  async function verify() {
    ...
    const users = loadData("users") || {};
    users[user.phone].activated = true;
    users[user.phone].activity = [{ text: "Account activated ✅", at: Date.now() }, ...(users[user.phone].activity || []).slice(0, 19)];
    saveData("users", users);
    setLoading(false);
    onActivated({ ...users[user.phone] });
  }
  ...
  return (
    <div style={S.overlay}>
      <div style={S.modal}>
        ...
        <p style={{ color: COLORS.muted, fontSize: 13, marginBottom: 20 }}>Pay KSH {ACTIVATION_FEE} once to unlock all earning features.</p>
        {step === 1 ? (
          <>
            ...
            <a href={PAYMENT_LINK} target="_blank" rel="noreferrer" style={{ display: "block", marginBottom: 10 }}>
              <button style={{ ...S.accentBtn, width: "100%" }}>💳 Pay via Lipwa →</button>
            </a>
            <button style={{ ...S.ghostBtn, width: "100%", marginBottom: 10 }} onClick={() => setStep(2)}>I've Paid — Enter M-Pesa Code</button>
            <button style={{ color: COLORS.muted, background: "none", border: "none", cursor: "pointer", width: "100%", fontSize: 13 }} onClick={onClose}>Cancel</button>
          </>
        ) : (
          <>
            <label style={S.label}>M-Pesa Confirmation Code</label>
            <input style={S.input} placeholder="e.g. QHJ4K38P2L" value={txCode} onChange={(e) => setTxCode(e.target.value)} />
            <button style={{ ...S.accentBtn, width: "100%", opacity: loading ? 0.7 : 1 }} onClick={verify} disabled={loading}>
              {loading ? "Verifying…" : "Verify & Activate ✓"}
            </button>
            <button style={{ ...S.ghostBtn, width: "100%", marginTop: 10 }} onClick={() => setStep(1)}>← Back</button>
          </>
        )}
      </div>
    </div>
    ...
    <div style={S.page}>
      ...
      <div style={S.card}>
        ...
        <div style={{ background: `${COLORS.accent}11`, border: `1px solid ${COLORS.accent}33`, borderRadius: 10, padding: "10px 14px", fontSize: 13 }}>
          🎁 Earn <strong style={{ color: COLORS.accent }}>KSH {REFERRAL_BONUS}</strong> for every friend who registers with your link!
        </div>
      </div>
      <div style={S.card}>
      ...
      {[
        ...
        <div key={t.name} style={S.taskCard}>
          ...
          <div style={{ textAlign: "right" }}>
            ...
            <button style={{ ...S.accentBtn, padding: "7px 14px", fontSize: 12, marginTop: 6, display: "block", opacity: t.done ? 0.5 : 1 }} onClick={t.action}>{t.btnLabel}</button>
          </div>
        </div>
      ))}
  ...
  return (
    <>
      ...
      <div style={S.app}>
        ...
        {showActivation && <ActivationModal user={user} onActivated={(u) => { setUser(u); setShowActivation(false); toast("🎉 Activated! Happy earning!"); }} onClose={() => setShowActivation(false)} />}
        {toastMsg && <Toast msg={toastMsg} onDone={() => setToastMsg(null)} />}
      </div>
    </>
  );
