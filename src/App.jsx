...
function loadIntaSendSDK() {
  ...
  _sdkPromise = new Promise((resolve, reject) => {
    if (window.IntaSend) { resolve(); return; }
    const s = document.createElement("script");
    s.src = "https://unpkg.com/[email protected]/build/intasend-inline.js";
    s.onload = () => resolve();
...
// ── Activation Modal ──────────────────────────────────────────────────────────
// IMPORTANT: Replace the value below with your real IntaSend publishable key.
// Get it from https://payment.intasend.com → Settings → API Keys
// Set INTASEND_LIVE = true when you are ready to accept real M-Pesa payments.
const INTASEND_PUBLIC_KEY = "YOUR_INTASEND_PUBLISHABLE_KEY";
const INTASEND_LIVE = false; // ← change to true in production

function ActivationModal({ user, onClose, onActivated }) {
  const [step, setStep] = useState("confirm"); // confirm | awaiting | success | failed
  const [errorMsg, setErrorMsg] = useState("");
  const btnRef = useRef(null);
  const intasendRef = useRef(null);

  // Normalize phone: 07XX → 2547XX
  const phone254 = user.phone.startsWith("0")
    ? "254" + user.phone.slice(1)
    : user.phone;

  // When step becomes "awaiting", load the SDK and wire up events
  useEffect(() => {
    ...
    loadIntaSendSDK()
      .then(() => {
        ...
        if (intasendRef.current) {
          try { intasendRef.current.off && intasendRef.current.off(); } catch(_) {}
        }
        const instance = new window.IntaSend({
          publicAPIKey: ISPubKey_live_ca26ac0e-ad9b-4079-bc91-bf0b64257b17,
          live: INTASEND_LIVE,
        })
          .on("COMPLETE", (response) => {
            if (cancelled) return;
            console.log("IntaSend COMPLETE:", response);
            // ✅ Payment verified — activate account NOW
            doActivate();
          })
          .on("FAILED", (response) => {
            if (cancelled) return;
            console.log("IntaSend FAILED:", response);
            setErrorMsg("Payment failed or was cancelled. Please try again.");
            setStep("failed");
          })
          .on("IN-PROGRESS", () => { console.log("IntaSend: in progress…"); });

        intasendRef.current = instance;

        // Auto-trigger the hidden IntaSend button
        setTimeout(() => {
          if (!cancelled && btnRef.current) btnRef.current.click();
        }, 400);
      })
      .catch(() => {
        if (!cancelled) {
          setErrorMsg("Could not load payment SDK. Check your internet connection.");
          setStep("failed");
        }
      });
  ...
  function doActivate() {
    ...
    const updated = {
      ...
      activated: true,
      activity: [
        { text: "Account activated via M-Pesa ✅", at: Date.now() },
        ...(user.activity || []).slice(0, 19),
      ],
    };
    if (user.referredBy) {
      const referrer = Object.values(users).find((u) => u.refCode === user.referredBy);
      if (referrer) {
        referrer.balance = (referrer.balance || 0) + REFERRAL_BONUS;
        referrer.referrals = (referrer.referrals || 0) + 1;
        referrer.activity = [
          { text: `Referral bonus +KSH ${REFERRAL_BONUS} 🎁`, at: Date.now() },
          ...(referrer.activity || []).slice(0, 19),
        ];
  ...
  return (
    <div style={S.overlay}>
      <div style={S.modal}>
        ...
        {step === "confirm" && (
          <>
            ...
            <p style={{ color: COLORS.muted, fontSize: 13, marginBottom: 20 }}>
              Pay <strong style={{ color: COLORS.gold }}>KSH {ACTIVATION_FEE}</strong> via M-Pesa STK Push.
              You will get a PIN prompt on your phone. Your account activates{" "}
              <strong style={{ color: COLORS.accent }}>only after M-Pesa confirms payment</strong>.
            </p>
            <div style={{ ...S.card, background: "#0d1424", marginBottom: 16 }}>
              <div style={{ fontSize: 12, color: COLORS.muted, marginBottom: 4 }}>M-Pesa Number</div>
              <div style={{ fontSize: 17, fontWeight: 700 }}>{user.phone}</div>
              <div style={{ fontSize: 12, color: COLORS.muted, marginTop: 8, marginBottom: 4 }}>Amount</div>
              <div style={{ fontSize: 28, fontWeight: 800, color: COLORS.gold }}>KSH {ACTIVATION_FEE}</div>
            </div>
            <button
              style={{ ...S.accentBtn, width: "100%", marginBottom: 10 }}
        ...
        {step === "awaiting" && (
          <>
            {/* Hidden IntaSend trigger — SDK finds this by class name */}
            <button
              ref={btnRef}
              className="intaSendPayButton"
              data-amount={ACTIVATION_FEE}
              data-currency="KES"
              data-method="M-PESA"
              data-phone_number={phone254}
              data-first_name={user.name.split(" ")[0] || "User"}
              data-last_name={user.name.split(" ").slice(1).join(" ") || ""}
              data-api_ref={`EH-${user.phone}-${Date.now()}`}
              style={{ display: "none" }}
            >Pay</button>
            <div style={{ textAlign: "center", padding: "16px 0" }}>
              <div style={{ fontSize: 44, marginBottom: 12 }}>📱</div>
              <div style={{ fontWeight: 800, fontSize: 17, marginBottom: 8 }}>Check Your Phone</div>
              <p style={{ fontSize: 13, color: COLORS.muted, margin: "0 0 16px", lineHeight: 1.6 }}>
                An M-Pesa STK Push has been sent to{" "}
                <strong style={{ color: COLORS.text }}>{user.phone}</strong>.{" "}
                Enter your <strong style={{ color: COLORS.accent }}>M-Pesa PIN</strong> to complete payment.
              </p>
              <div style={{
                ...
                ⏳ Waiting for M-Pesa confirmation… Do not close this screen.
              </div>
              <button style={{ ...S.ghostBtn, width: "100%", fontSize: 12 }} onClick={onClose}>
                Cancel
              </button>
            </div>
          </>
        ...
        {step === "success" && (
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 20, marginBottom: 8, color: COLORS.accent }}>
              Payment Confirmed!
            </div>
            <p style={{ fontSize: 13, color: COLORS.muted, margin: "0 0 20px", lineHeight: 1.6 }}>
              M-Pesa payment received. Your EarnHub account is now{" "}
              <strong style={{ color: COLORS.accent }}>fully activated</strong>. Time to earn!
            </p>
            <button style={{ ...S.accentBtn, width: "100%" }} onClick={onClose}>
              Start Earning 🚀
            </button>
          </div>
        )}
        ...
        {step === "failed" && (
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 44, marginBottom: 12 }}>❌</div>
            <div style={{ fontWeight: 800, fontSize: 18, marginBottom: 8, color: COLORS.red }}>
              Payment Failed
            </div>
            <p style={{ fontSize: 13, color: COLORS.muted, margin: "0 0 20px" }}>{errorMsg}</p>
            <button
              style={{ ...S.accentBtn, width: "100%", marginBottom: 10 }}
  ...
  return (
    <>
      ...
      <div style={S.app}>
        ...
        {showActivation && <ActivationModal user={user} onClose={() => setShowActivation(false)} onActivated={(u) => { setUser(u); setShowActivation(false); toast("🎉 Account Activated! Start earning now!"); }} />}
        {toastMsg && <Toast msg={toastMsg} onDone={() => setToastMsg(null)} />}
      </div>
    </>
  );
