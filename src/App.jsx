...
function ActivationModal({ user, onClose, onActivated }) {
  ...
  function handlePayClick() {
    ...
    const users = loadData("users") || {};
    const updated = { ...user, activated: true, activity: [{ text: "Account activated ✅", at: Date.now() }, ...(user.activity || []).slice(0, 19)] };

    // Credit referral bonus to whoever referred this user
    if (user.referredBy) {
      ...
      if (referrer) {
        ...
        referrer.activity = [{ text: `Referral bonus +KSH ${REFERRAL_BONUS} 🎁`, at: Date.now() }, ...(referrer.activity || []).slice(0, 19)];
    ...
    // Open payment link and show success
      ...
      <div style={S.modal}>
        {step === "confirm" && (
          <>
            ...
            <p style={{ color: COLORS.muted, fontSize: 13, marginBottom: 20 }}>
              Pay KSH {ACTIVATION_FEE} once to unlock all earning features. Your account activates{" "}
              <strong style={{ color: COLORS.accent }}>immediately</strong> when you tap Pay.
            </p>
        ...
        {step === "done" && (
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 20, marginBottom: 8, color: COLORS.accent }}>Account Activated!</div>
            <p style={{ fontSize: 13, color: COLORS.muted, margin: "0 0 20px" }}>
              Your EarnHub account is now <strong style={{ color: COLORS.accent }}>fully active</strong>. Complete the payment on the Lipwa page that just opened. You can now earn and withdraw!
            </p>
